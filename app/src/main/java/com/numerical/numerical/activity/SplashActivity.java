...
public class SplashActivity extends AppCompatActivity {
    ...
    private void requestPermission() {
        ActivityCompat.requestPermissions(this, new String[]{ACCESS_FINE_LOCATION,GET_TASKS,READ_EXTERNAL_STORAGE,WRITE_EXTERNAL_STORAGE, " "}, PERMISSION_REQUEST_CODE);
    }

    private boolean checkPermission() {
        int result = ContextCompat.checkSelfPermission(getApplicationContext(), ACCESS_FINE_LOCATION);
        int result1 = ContextCompat.checkSelfPermission(getApplicationContext(), GET_TASKS);
            ...
            case PERMISSION_REQUEST_CODE:
                if (grantResults.length > 0) {
                 ...
                 /*   boolean ContactAccepted = grantResults[2] == PackageManager.PERMISSION_GRANTED;
                    ...
                    if (locationAccepted && cameraAccepted && ContactAccepted && ExtStorageAccepted ) {
                        checkTimerIntent();
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                            if (shouldShowRequestPermissionRationale(ACCESS_FINE_LOCATION)) {
                                ...
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                    requestPermissions(new String[]{ACCESS_FINE_LOCATION, READ_EXTERNAL_STORAGE, WRITE_EXTERNAL_STORAGE}, PERMISSION_REQUEST_CODE);
                                    checkTimerIntent();
                                }
                            }
                        }
